...
These classes only checks the required kernel configuration options. No automatic modifications to the kernel configuration are made.

In addition, these classes allows to customize the autoloading of the kernel modules in case of that they builded as modules in kernel.
...
In the optional `version` parameter it is possible to specify the versions of the Linux kernel for which the checks will be performed.
