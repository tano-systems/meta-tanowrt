...
do_deploy_append() {
	if [ "${UBOOT_SPL_MULTIPLE}" = "1" ]; then
		rm -f ${DEPLOYDIR}/${UBOOT_SPL_MULTIPLE_OUTPUT}

		if [ ! -e ${DEPLOYDIR}/${UBOOT_SPL_MULTIPLE_INPUT} ]; then
			bbfatal "Could not find input SPL binary at ${DEPLOYDIR}/${UBOOT_SPL_MULTIPLE_INPUT}"
		fi

		local INPUT_SPL_SIZE=$(stat -L -c %s ${DEPLOYDIR}/${UBOOT_SPL_MULTIPLE_INPUT} 2>/dev/null)
		...
		for SPL_COUNT in ${UBOOT_SPL_MULTIPLE_COUNT}; do
			...
			dd if=/dev/zero \
				...
				| tr "\000" "${UBOOT_SPL_MULTIPLE_PADDING_BYTE}" \
					> ${DEPLOYDIR}/${UBOOT_SPL_MULTIPLE_OUTPUT}-x${SPL_COUNT}

			local N=0
			while [ ${N} -lt ${SPL_COUNT} ]; do
				local SEEK=$(echo "${UBOOT_SPL_MULTIPLE_BLOCK_SIZE} * ${N}" | bc)
				dd if=${DEPLOYDIR}/${UBOOT_SPL_MULTIPLE_INPUT} \
				   of=${DEPLOYDIR}/${UBOOT_SPL_MULTIPLE_OUTPUT}-x${SPL_COUNT} \
				   ibs=${INPUT_SPL_SIZE} \
				   obs=1 \
				   count=1 \
				   conv=notrunc \
			...
			ln -sf ${UBOOT_SPL_MULTIPLE_OUTPUT}-x${SPL_COUNT} \
			       ${DEPLOYDIR}/${UBOOT_SPL_MULTIPLE_OUTPUT_LINK}-x${SPL_COUNT}
		done
	fi
}
