...
openwrt_flatten_modules_hook () {
    ...
    if [ -d "${IMAGE_ROOTFS}/lib/modules/${KERNEL_VERSION}" ]; then
        cd ${IMAGE_ROOTFS} && find lib/modules/${KERNEL_VERSION} -name '*.ko' -exec sh -c \
            'mod="{}"; target=${IMAGE_ROOTFS}/lib/modules/${KERNEL_VERSION}/$(basename "$mod"); if [ ! -e "$target" ]; then ln -sf /$mod $target; fi' \;
    fi
}
